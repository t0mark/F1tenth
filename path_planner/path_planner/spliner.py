...
class SplineNode(Node):
    def __init__(self):
        ...
        self.pre_apex_0 = -2.0   # [m] 첫 번째 진입점: 장애물 3m 앞에서 회피 경로 시작
                                 # - 감소(예: -2.0) → 늦게 회피 시작, 급격한 조향
                                 # - 증가(예: -4.0) → 일찍 회피 시작, 부드러운 조향

        self.pre_apex_1 = -1.25   # [m] 두 번째 진입점: 장애물 1.5m 앞
        ...
        self.post_apex_1 = 1.25  # [m] 두 번째 복귀점: 장애물 1.25m 뒤
                                 # - post_apex_0과의 간격이 복귀 급격도 결정

        self.post_apex_2 = 2.0   # [m] 마지막 복귀점: 장애물 2m 뒤, 센터라인 완전 복귀
