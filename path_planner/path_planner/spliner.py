...
class SplineNode(Node):
    def __init__(self):
        ...
        # ==================== 회피 경로 스플라인 파라미터 ====================
        # 회피 경로는 7개 제어점으로 3차 스플라인을 생성합니다:
        # [pre_apex_0, pre_apex_1, pre_apex_2, 0(apex), post_apex_0, post_apex_1, post_apex_2]
        #
        # 스플라인 형상 조정 가이드:
        #   - 값 감소 → 급격한 회피 (공격적, 빠른 반응, 높은 횡가속도)
        #   - 값 증가 → 완만한 회피 (보수적, 느린 반응, 안정적)
        # =====================================================================
        ...
        self.pre_apex_0 = -4.0   # [m] 첫 번째 진입점 기준값 (작은 맵용: 절반으로 축소)
                                 # - 음수 = 장애물 apex 기준 앞쪽 거리
                                 # - 더 음수로(예: -2.0) → 더 일찍 회피 시작, 완만한 곡선
                                 # - 0에 가깝게(예: -1.0) → 늦게 회피 시작, 급격한 곡선

        self.pre_apex_1 = -2.5   # [m] 두 번째 진입점 기준값 (작은 맵용: 절반으로 축소)
                                 # - pre_apex_0과의 간격이 진입 곡선의 급격도 결정
                                 # - 간격 좁을수록 급격한 초기 회피

        self.pre_apex_2 = -1.5  # [m] 세 번째 진입점 기준값 (작은 맵용: 절반으로 축소)
                                 # - apex 직전 마지막 제어점
                                 # - 최종 회피 각도 결정

        # apex (0m): 장애물과 나란히 있을 때 최대 횡방향 변위 지점 (자동 계산)

        self.post_apex_0 = 1.5   # [m] 첫 번째 복귀점 기준값 (작은 맵용: 축소)
                                 # - 양수 = 장애물 apex 기준 뒤쪽 거리
                                 # - 0에 가깝게(예: 0.3) → 빠른 복귀, 공격적
                                 # - 더 크게(예: 0.9) → 늦은 복귀, 안전

        self.post_apex_1 = 2.5  # [m] 두 번째 복귀점 기준값 (작은 맵용: 축소)
                                 # - post_apex_0과의 간격이 복귀 곡선의 급격도 결정

        self.post_apex_2 = 4.0   # [m] 마지막 복귀점 기준값 (작은 맵용: 절반으로 축소)
        ...
        self.outside_overtake_scale = 1.75  # [배수] 외측 추월 시 스플라인 길이 스케일링
                                           # - 1.0 = 내측과 동일 (급격한 회피)
                                           # - 1.3 = 작은 맵용 (30% 더 완만)
        ...
        self.evasion_dist = 0.7        # [m] 장애물과의 횡방향(lateral) 최소 안전 거리
                                        # - 감소(예: 0.15) → 장애물에 더 가까이, 좁은 트랙 유리
                                        # - 증가(예: 0.4) → 안전 여유 확보, 충돌 위험 감소
                                        # ⚠️ 너무 크면 트랙 경계 침범으로 회피 실패!

        self.spline_bound_mindist = 0.05 # [m] 트랙 경계로부터 유지할 최소 안전 마진
                                        # - 0.0 = 트랙 전체 폭 사용 (공격적)
                                        # - 증가(예: 0.1) → 경계에서 여유 확보 (보수적)
                                        # ⚠️ 값이 클수록 회피 가능 공간 감소

        self.gb_ego_width_m = 0.2      # [m] 차량 폭의 절반 (차량 중심에서 양쪽까지의 거리)
                                        # - 레이스라인 복귀 판단 및 경계 마진 계산에 사용

        self.lateral_width_m = 0.4      # [m] 장애물 회피 시 횡방향 여유 거리
                                        # - 장애물과 충돌 여부 판단 기준
                                        # - 추월 apex 위치 계산에 사용

        # ==================== 장애물 감지 및 필터링 ====================

        self.obs_traj_tresh = 0.325      # [m] 레이싱 라인 중심에서 이 거리 이내 장애물만 고려
                                        # - 감소(예: 0.2) → 센터라인 근처 장애물만 회피
        ...
        self.fixed_pred_time = 0.15     # [s] 장애물의 미래 위치 예측 시간
                                        # - 동적 장애물의 이동 경로 예측용

        self.kd_obs_pred = 1.0          # 장애물 횡방향 속도 예측 게인
                                        # - 현재 코드에서 주석처리되어 미사용
