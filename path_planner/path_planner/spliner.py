...
class SplineNode(Node):
    def __init__(self):
        ...
        self.pre_apex_0 = -2.3   # [m] 첫 번째 진입점 기준값 (작은 맵용: 절반으로 축소)
                                 # - 음수 = 장애물 apex 기준 앞쪽 거리
                                 # - 더 음수로(예: -2.0) → 더 일찍 회피 시작, 완만한 곡선
                                 # - 0에 가깝게(예: -1.0) → 늦게 회피 시작, 급격한 곡선

        self.pre_apex_1 = -1.8  # [m] 두 번째 진입점 기준값 (작은 맵용: 절반으로 축소)
                                 # - pre_apex_0과의 간격이 진입 곡선의 급격도 결정
                                 # - 간격 좁을수록 급격한 초기 회피

        self.pre_apex_2 = -1.25   # [m] 세 번째 진입점 기준값 (작은 맵용: 절반으로 축소)
                                 # - apex 직전 마지막 제어점
                                 # - 최종 회피 각도 결정
        ...
        self.evasion_dist = 0.45        # [m] 장애물과의 횡방향(lateral) 최소 안전 거리
    ...
    def do_spline(self, obstacles: ObstacleArray) -> Tuple[WpntArray, MarkerArray]:
        ...
        if len(close_obs) > 0:
            ...
            spatial_spline = Spline(x=s_coords, y=evasion_points[:, 1], k=4)   # 3차 스플라인
