...
class SplineNode(Node):
    def __init__(self):
        ...
        self.pre_apex_0 = -3.0   # [m] 첫 번째 진입점: 장애물 3m 앞에서 회피 경로 시작
                                 # - 감소(예: -2.0) → 늦게 회피 시작, 급격한 조향
                                 # - 증가(예: -4.0) → 일찍 회피 시작, 부드러운 조향

        self.pre_apex_1 = -1.5   # [m] 두 번째 진입점: 장애물 1.5m 앞
        ...
        self.post_apex_0 = 1.5  # [m] 첫 번째 복귀점: 장애물 0.75m 뒤, 센터라인 복귀 시작
                                 # - 감소(예: 0.5) → 빠른 복귀, 공격적
                                 # - 증가(예: 1.5) → 늦은 복귀, 안전

        self.post_apex_1 = 2.0  # [m] 두 번째 복귀점: 장애물 1.25m 뒤
                                 # - post_apex_0과의 간격이 복귀 급격도 결정

        self.post_apex_2 = 2.5   # [m] 마지막 복귀점: 장애물 2m 뒤, 센터라인 완전 복귀
                                 # - 회피 경로의 끝점, 작을수록 빠른 최적 라인 복귀
                                 # - 너무 작으면 복귀 중 불안정, 너무 크면 랩타임 손실

        # ==================== 안전 거리 파라미터 ====================

        self.evasion_dist = 0.35        # [m] 장애물과의 횡방향(lateral) 최소 안전 거리
